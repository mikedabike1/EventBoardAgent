...
class FacebookEventScraper(EventExtractor):
    ...
    def scrape_page_events(self, page_id: str) -> List[GamingEvent]:
        ...
        if not self.access_token:
            print(
                "Warning: No Facebook access token provided. Using public scraping fallback."
            )
        ...
        try:
            ...
            for line in lines:
                if self.contains_gaming_keywords(line):
                    event = self._parse_text_event(
                        line, f"https://facebook.com/{page_id}"
                    )
                    if event:
                        events.append(event)
    ...
    def _parse_facebook_event(
        self, event_data: Dict, source_url: str
    ) -> Optional[GamingEvent]:
