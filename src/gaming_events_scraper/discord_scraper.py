...
class DiscordEventScraper(EventExtractor):
    ...
    async def scrape_server_events(self, guild_id: int, channel_names: Optional[List[str]] = None) -> List[GamingEvent]:
    ...
    def _parse_discord_message(self, message) -> Optional[GamingEvent]:
        ...
        try:
            ...
            return GamingEvent(
                ...
                source="discord",
                source_url=f"https://discord.com/channels/{message.guild.id}/{message.channel.id}/{message.id}" if hasattr(message, 'guild') and message.guild else None,
                description=content
            )
